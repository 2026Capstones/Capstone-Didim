...
## 6단계: AI 면접 UI/UX Re-design (Front)
기존의 작은 화면과 단편적인 녹화 방식을 개선하여, 연속적이고 몰입감 있는 면접 환경을 제공합니다.

- [x] **팝업(Modal) 기반 UI 렌더링:** 면접 시작 시 전체 화면을 덮는 팝업을 띄우고, 좌측에 크게 카메라 화면을 배치합니다.
- [x] **타이머 로직 추가:** 우측 상단에 60초 타이머를 배치하고, 녹화 시작과 함께 1초씩 감소시킵니다. (0초 도달 시 자동 전송)
- [x] **꼬리 질문 연속성 보장:** 답변 1회 제출 후 카메라 스트림을 종료하지 않고 유지하며, 서버에서 새 질문을 받으면 타이머를 초기화하고 녹화를 재개합니다.
- [x] **데이터 전송 방식 유지 검증:** 백엔드의 Multipart/form-data 요구사항에 맞춰, 질문 내용 추가 없이 기존 미디어 파일 전송 방식을 그대로 사용합니다.
